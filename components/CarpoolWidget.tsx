...
export default function CarpoolWidget({
  ...
  const { data: passengers, isLoading: isLoadingPassengers } = useGetPassengersByCarpoolId(
    carpool.id
  );
  ...
  const driver = users?.data.find((u: { id: number }) => u.id === carpool.driverId);
  ...
  return (
    <div>
      <Stack>
        ...
        <List>
          {passengers?.data.data.map((passenger: { id: number; passengerId: number }) => {
      ...
      {!isDriver && (
        <Group mt={40}>
          {selfPassenger ? (
            ...
            <Button
              onClick={() => addPassenger.mutate(carpool.id)}
              disabled={!participation || carpool.seats <= passengers?.data.data.length}
            >
      ...
      <Modal opened={deleteOpened} onClose={closeDelete} title="Poista kyyti" fullScreen={isMobile}>
        ...
        <Group>
          ...
          <Button
            color="red"
            onClick={() => deleteCarpool.mutate(carpool.id)}
            disabled={passengers?.data.data.length > 0}
          >
            Poista
          </Button>
        </Group>
